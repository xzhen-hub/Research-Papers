...
/***Updated 3/2/2021:Add t-test for treatment and control groups*******/
gen postinter=(cyear>=inter) & !missing(cyear)	   
		   
ssc install diff
 diff investment2, t(dinter) p(postinter)

 /****
 ttest investment2, by(dinter) unequal

 ttest investment2, by(dinter)

 ttest investment2 , by (dinter)
 estpost ttest investment, by(dinter)

 ssc install asdoc
 foreach var of varlist investment2 {
         asdoc ttest `var', by(dinter) rowappend
}
 **/
 estpost ttest investment2 cash_flow2 firmsize tobin_q1 cashw stock_issues lt_debt, by(dinter)
 esttab
  esttab using ttestfordid.csv, replace wide plain
  
  
esttab , noobs cells("b(star fmt(4)) t(fmt(4)) se(fmt(4)) count(fmt(0))") star(* 0.1 ** .05 *** 0.01) ///
collabels("Diff." "t" "Std. Error" "Obs.")
 
 
 estpost ttest investment2 cash_flow2 firmsize tobin_q1 cashw stock_issues lt_debt, by(dinter)
 
/******for inter deregulation************/
estpost sum investment2 cash_flow2 firmsize tobin_q1 cashw stock_issues lt_debt if dinter==1
matrix meani1=e(mean)
matrix list meani1

estpost sum investment2 cash_flow2 firmsize tobin_q1 cashw stock_issues lt_debt if dinter==0
matrix meani0=e(mean)
matrix list meani0

estpost ttest investment2 cash_flow2 firmsize tobin_q1 cashw stock_issues lt_debt, by(dinter)

estadd matrix meani1
estadd matrix meani0

esttab , noobs cells("meani1(fmt(4)) meani0(fmt(4)) b(star fmt(4)) se(fmt(4)) t(fmt(4)) count(fmt(0))") star(* 0.1 ** .05 *** 0.01) ///
collabels("Mean(Treatment group)" "Mean(Control group)" "Diff." "Std. Error" "t" "Obs.")
 
 
 /******for intra deregulation************/
estpost sum investment2 cash_flow2 firmsize tobin_q1 cashw stock_issues lt_debt if dintra==1
matrix meanf1=e(mean)
matrix list meanf1

estpost sum investment2 cash_flow2 firmsize tobin_q1 cashw stock_issues lt_debt if dintra==0
matrix meanf0=e(mean)
matrix list meanf0

estpost ttest investment2 cash_flow2 firmsize tobin_q1 cashw stock_issues lt_debt, by(dintra)

estadd matrix meanf1
estadd matrix meanf0

esttab , noobs cells("meanf1(fmt(4)) meanf0(fmt(4)) b(star fmt(4)) se(fmt(4)) t(fmt(4)) count(fmt(0))") star(* 0.1 ** .05 *** 0.01) ///
collabels("Mean(Treatment group)" "Mean(Control group)" "Diff." "Std. Error" "t-statistic" "Obs.")
